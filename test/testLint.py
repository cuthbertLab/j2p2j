...
import j2p2j
j2p2jDir = os.path.dirname(j2p2j.__file__)
...
def main(fnAccept=None, strict=False):
    ...
    mg = glob.glob(j2p2jDir + '/**/*.py', recursive=True)

    fnPathReject = ['seed', 'newChanges', 'OLD_j2p2j']
...
def argRun():
    parser = argparse.ArgumentParser(description='Run pylint on j2p2j according to style guide.')
